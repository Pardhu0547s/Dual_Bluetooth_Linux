...
/// Linux audio service using PipeWire virtual sinks for dual Bluetooth streaming.
class LinuxAudioService extends AudioService {
  ...
  @override
  String get platformName => 'Linux (PipeWire)';
  ...
  Future<bool> startDualStream({
    ...
    try {
      ...
      // 3. Find Master Sink Node ID & set as default sink
      final dump = await Process.run('pw-dump', []);
      final List<dynamic> data = jsonDecode(dump.stdout as String);
      int? masterNodeId;
      for (final item in data) {
  ...
  void _startDisconnectMonitor(AudioSink target1, AudioSink target2) {
    ...
    _disconnectTimer = Timer.periodic(const Duration(seconds: 2), (_) async {
      ...
      if (!t1Active || !t2Active) {
        ...
        print('Device disconnected: $disconnectedName. Stopping dual stream.');
        await stopDualStream();
